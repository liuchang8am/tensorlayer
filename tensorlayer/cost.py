...
def sparse_cross_entropy(output, target, name=None):
    # same as cross_entropy, but target should be [batch_size], where each element is a single class number
...
def cross_entropy(output, target, name=None):
    # same as sparse_cross_entropy, but target should be [batch_size, num_classes]
    return tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=output, labels=target, name=name))
